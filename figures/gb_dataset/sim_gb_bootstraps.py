...
pd = {}
sampling_matrix = np.loadtxt(Path(f"{data_dir}/GB_v54.1_{genodata_type}_sample_sizes.table"), dtype=int)
...
for sel_type in ["add", "dom", "het", "rec"]:
    temp_seed += 1
    with open(Path(f"{output_dir}/GB_v54.1_{genodata_type}_{sel_type}_sig_windows.pkl"), "rb") as file:
    ...
    for _, lead_snp in lead_snps_table.iterrows():
        chr = lead_snp["Chr."]
        chr_idx = lead_snp["SNP. index of max (on chr)."]
        real_data_path = Path(f"{data_dir}/GB_v54.1_{genodata_type}_c{chr}.csv")
        ...
        sel_str = lead_snp[r"$\hat{s}(p_{min})"]
        if sel_type == "het":
            if sel_str > 0:
                true_sel_type = "over"
            else:
        ...
        p_init = real_data_file["all_means"][lead_snp["SNP index of max."]]
