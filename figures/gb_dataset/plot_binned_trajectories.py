...
complete_agg_data_path = Path(f"../runs/{subdir_name}/datasets/tcec_superfinal/UK_v54.1_capture_only_complete_agg_data.pkl")
with open(complete_agg_data_path, "rb") as file:
    cdata = pickle.load(file)

for c_type in c_types:
    with open(f"../runs/{subdir_name}/datasets/tcec_superfinal/UK_v54.1_{genodata_type}_{c_type}_brown_windows.pkl",
              "rb") as file:
...
binned_path = Path(f"../runs/{subdir_name}/datasets/tcec_superfinal/UK_v54.1_{genodata_type}_binned.csv")
...
for l_i, lead_snp in enumerate(lead_snp_idxs):
    ...
    fig.savefig(Path(f"../runs/{subdir_name}/paper_figures/tcec_superfinal/frequency_trajectories/chr{int(cdata['all_chrom'][lead_snp])}_{lead_snp}_{cdata['all_rsid'][lead_snp]}.pdf"), format="pdf", bbox_inches="tight")