...
1. Run `emsel-sim data/pure_sim -s .005 .01 .025 .05 -g 101 251 1001 -ic recip .005 .25 --sel_types neutral add dom rec over under --seed 5 -n 1000`.
2. Run `python SLURM_example.py` with `EM_dir = Path('EM/pure_sim')` and `data_dir = Path('data/pure_sim')` at the beginning of the script, modifying the other parameters to your liking. This should generate 177 files to be run. Then, run `sh meta_sim_EM.sh` to submit the jobs to the cluster. Alternatively, for each dataset in `data`, running `emsel data/pure_sim/{file_name}_data.csv EM/pure_sim/{file_name}_EM --time_after_zero -maf 0 --min_sample_density 0 --full_output`.
...
### Figures 3D, 4, S.1-8D:
First, fit the generalized gamma distribution to the unconstrained EM for each dataset. To do this, set the following parameters at the beginning of the script `permute_target_dataset.py` and run it using `python permute_target_dataset.py`:
```
num_gens_list = [101, 251, 1001]
init_dists = [.005, .25, "recip"]
...
4. Run `python interpolate_Ne_grid.py` with `EM_dir = "EM/ibdne"` at the beginning of the script. This should output `Estimated Ne: 9987`. If it does not, modify the line `Ne = 9987` appears in `SLURM_example.py` to the correct value.
...
Lastly, set the following parameters at the beginning of the script `d2_qqs_and_confusiontables.py` and run it using `python d2_qqs_and_confusiontables.py`:
```
sel_strs = [.005, .01, .025, .05]
num_gens_list = [125]
init_dists = ["real_special"]
