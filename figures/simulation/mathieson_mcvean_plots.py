...
import os
import glob
from util import plot_one_ll_grid, params_dict_to_str, convert_from_abbrevs
from cycler import cycler
from copy import deepcopy

from util import calculate_converge_idx
...
spikesandslabs = np.linspace(0, 1, 11)
...
init_dists = [.25]
condos = [True]
#sel_strs = [.005, .01, .025, .1]
#num_gens_list = [101, 251, 1001]
#init_dists = ["fbeta", .002, .4]
#condos = [True, False]

subdir_name = "tree/sim"


image_path = f"../runs/{subdir_name}/paper_figures/finalfinal/"
...
#//2 to convert from 2Ne to Ne
knee_sci_ticks = [f"{knees[i]//2:.0e}" for i in range(len(knees))]
...
for v_i, var_list in enumerate(parameter_lists):
    ...
    v_internal = 0
    for sel_str, init_dist, num_gens, var in itprod(sel_strs, init_dists, num_gens_list, var_list):
        fname_dict = {}
        ...
        if v_i != 2:
            ...
            fname_dict[f"{var_names[v_i]}"] = var*2
        ...
        for sel_i, sel_type in enumerate(onep_sel_types):
            ...
            hmm_filename = Path(f"../runs/{subdir_name}/EM/mathieson/{hmm_name}_linapprox_nbeta_final_EM.bz2")
            # if v_i == 2:
            #     hmm_filename = Path(f"../runs/{subdir_name}/EM/{hmm_name}_new_EM.bz2")
            print(hmm_filename)
    ...
    for line_i in np.arange(data_array.shape[0]):
        ...
        #axs[v_i//2, v_i%2].fill_between(var_list, ci_array[line_i, :, 0], ci_array[line_i, :, 1], alpha=.1)
...
fig.savefig(image_path+f"param_variation_plot_linapprox_final.pdf", format="pdf", bbox_inches="tight")
#fig.savefig(image_path+f"param_variation_plot_linapprox_final.png", format="png", bbox_inches="tight")
plt.close(fig)