...
neutral_hmm_path = Path(f"../runs/{subdir_name}/EM/{neutral_filename}_linapprox_nbeta_final_EM.bz2")
...
add_em_axins = add_em_axs.inset_axes([.65, .11, .3, .3])
logps = []
labels = []
for type_i, run_type in enumerate(sel_types):
    ...
    for str_i, sel_str in enumerate(sel_strs):
        ...
        nn_path = Path(f"../runs/{subdir_name}/EM/{nn_fname}_linapprox_nbeta_final_EM.bz2")
...
plot_qq(add_em_axs, add_em_axins, logps, labels)
add_em_fig.savefig(Path(f"../runs/{subdir_name}/paper_figures/finalfinal/llr_plots/neutral_add_em_llr_all.pdf"), format="pdf", bbox_inches="tight")
...
add_em_auc_fig.savefig(Path(f"../runs/{subdir_name}/paper_figures/finalfinal/auc_plots/{big_ts_str}_add_em_auc_plot.pdf"), format="pdf", bbox_inches="tight")
...
for type_i, run_type in enumerate(sel_types):
    ...
    logps.append(full_p_vals)
    labels.append(convert_from_abbrevs(run_EM_str))
plot_qq(add_sim_axs, add_sim_axins, logps, labels)
add_sim_fig.savefig(Path(f"../runs/{subdir_name}/paper_figures/finalfinal/llr_plots/neutral_add_sim_llr_all.pdf"), format="pdf", bbox_inches="tight")
...
add_sim_auc_fig.savefig(Path(f"../runs/{subdir_name}/paper_figures/finalfinal/auc_plots/{big_ts_str}_add_sim_auc_plot.pdf"), format="pdf", bbox_inches="tight")
