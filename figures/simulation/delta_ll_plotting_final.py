...
from util import classify_full_run, bh_correct, get_one_count_matrix, params_dict_to_str, get_llg_array, get_lr_statistic, get_llgka_array, full_bh_procedure, classify_llgk_array, convert_from_abbrevs, convert_to_abbrevs, plot_qq
...
save_bh = True if p_val == .05 else False
save_bh = True
    ...
    for init_dist in init_dists:
        ...
        for sel_str in sel_strs:
            ...
            for sel_i, sel_type in enumerate(sel_types):
                ...
                onep_path = Path(f"../runs/{subdir_name}/EM/{hmm_prefix_str}{exp_name}_{hmm_fname_str}EM.bz2")
                if sel_type == "under" and not onep_path.is_file():
                    final_sel_types = ["add", "dom", "rec", "over"]
                    continue

                with BZ2File(onep_path, "rb") as file:
        ...
        #n_full[0] = n_init_str + n_full[0]
        n_full[n_max_idx] = n_full[n_max_idx]
        n_full[0] = n_full[0]

        indices = [f"Neutral"]
        if save_bh:
            tpath = Path(f"../runs/{subdir_name}/BH/finalfinal/{row_list[0][0]}_classified.bz2")
            bh_dict = {
                "bh_classes": test_vals[0],
                "p_vals": g_p_val_matrix[0],
                "p_cutoff": p_cutoff
            }
            with BZ2File(tpath, "wb") as file:
        ...
        if save_bh:
            for s_i, s_array in enumerate(test_vals):
                for col in np.arange(s_array.shape[1]):
                    tpath = Path(f"../runs/{subdir_name}/BH/finalfinal/{convert_to_abbrevs(row_list[s_i][col])}_s{str(sel_strs[s_i])[2:]}_g{num_gens}_d{str(init_dist)[2:]}_classified.bz2")
                    bh_dict = {
        ...
        for s_i, s_array in enumerate(test_vals):
            if sel_strs[s_i] in [0.005, 0.025]:
                continue
            print(np.arange(s_array.shape[1]))
            for col in np.arange(s_array.shape[1]):
        ...
        #fig_height = 4.5 if n_full_df.shape[0] == len(sel_types_rows) else 3.75
        ...
        fig2.savefig(Path(
            f"../runs/{subdir_name}/paper_figures/finalfinal/confusion_plots/{tablename}_confusion_plot.pdf"),
            format="pdf", bbox_inches="tight")
        # n_full_df["\\diagbox{Simulated}{Inferred}"] = indices[cut_idx:]
        #
        # cols = n_full_df.columns.to_list()
        # cols = cols[-1:] + cols[:-1]
        # n_full_df = n_full_df[cols]
        #
        # tablename = params_dict_to_str(**{"init_dist": init_dist, "num_gens": num_gens})
        # n_full_df.to_latex(f"../runs/{subdir_name}/tables/finalfinal/dll_{tablename}_p{str(p_val)[2:]}.tex", float_format="%.2f", escape=False, index=False)
        ...
        #axs[0].hist(n_lrs[n_lrs > lr_shift] - lr_shift, bins=50, label="hist", density=True, alpha=.5, color="k")
        #lr_space = np.linspace(0, np.amax(n_lrs - lr_shift), 500)

        axins = axs.inset_axes([.65, .11, .3, .3])
        axs.text(-.2, .97, r"$\bf{D}$", fontsize=13, transform=axs.transAxes)
        plot_qq(axs, axins, logps, labels, colors=colors, legend_loc = "upper left", thin=thinness)
        fig.savefig(Path(f"../runs/{subdir_name}/paper_figures/finalfinal/dll_plots/neutral_g{num_gens}_d{init_dist}_dll_all.pdf"), format="pdf", bbox_inches="tight")
        plt.close(fig)