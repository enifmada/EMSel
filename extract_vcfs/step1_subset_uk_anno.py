...
EXTRACTED_PREFIX = f"{OUTPUT_DIR}/UK_{AADR_VERSION}"
...
def extractUK (annoFrame):
    ...
    ukMask = entityMask & passMask & noRelativeMask
    annoFrame = annoFrame.loc[ukMask]

    print (f"{numpy.sum(ukMask)} individuals in UK that pass filter (and no relatives)")
...
def main ():
    ...
    # extract the UK individuals that we want to focus on
    annoUK = extractUK (anno)

    # ACTUALLY NOT TRUE ANYMORE
    # # for this particular dataset, we should have only ancient individuals
    # assert (annoUK['Date_mean'].min() > 0)

    # just check some things
    print (f"{annoUK.shape[0]} UK individuals in time frame")
    presentMask = (annoUK['Date_mean'] <= 0)
    ancientMask = (annoUK['Date_mean'] > 0)
    shotgunMask = numpy.isin (annoUK['Data_source'], ['Shotgun', 'Shotgun.diploid'])
    captureMask = numpy.isin (annoUK['Data_source'], ["1240K", "1240k"])
    print (collections.Counter(annoUK['Publication']))
    ...
    print (f"\t{annoUK[ancientMask & shotgunMask].shape[0]} shotgun; {annoUK[ancientMask & captureMask].shape[0]} capture")

    # # create the directory for the output
    # os.makedirs (OUTPUT_DIR)

    # and write the new table file
    writeAnnotation (annoUK, OUTPUT_TABLE_SG)

    # now, we also need to modify the *.ind file
    createMockIndFile (IND_FILE_HO, OUTPUT_IND_SG, annoUK)
